...
function SuccessScreen({ name, apiToken }: { name: string; apiToken: string }) {
  const origin = window.location.origin
  const loginCmd = `./cl login ${origin} ${apiToken}`
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
      <div className="max-w-xl mx-auto pt-12">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {PLATFORMS.map(p => (
              <a
                key={p.id}
                href={`${origin}/api/downloads/${p.id}`}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 mb-4">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Run this after <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">chmod +x ./cl</code></p>
        </div>
...
export default function InviteUse() {
  ...
  const [result, setResult] = useState<{ name: string; api_token: string } | null>(null)
  ...
  if (result) {
    return <SuccessScreen name={result.name} apiToken={result.api_token} />
  }
